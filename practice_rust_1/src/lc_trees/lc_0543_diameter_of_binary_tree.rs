...
impl Solution {
    ...
    fn diameter(
        ...
        if let Some(node) = root {
            let left_height =
                Self::height(&node.borrow().left);
            let righ_height =
                Self::height(&node.borrow().right);

            let left_diam =
                Self::diameter(&node.borrow().left);
            let right_diam =
                Self::diameter(&node.borrow().right);

            (left_height + righ_height)
                .max(left_diam.max(right_diam))
        } else {
            0
        }
    }
