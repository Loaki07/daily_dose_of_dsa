// Heap data structure
// Takes a closure as a comparator to allow for
// min-heap, max-heap and works with custom key
// functions
...
impl<T> Heap<T> {
    pub fn new(comparator: fn(&T, &T) -> bool) -> Self {
        Self {
            // Add a default in the first spot to offset
            // indexes for the parent/child math
            // to work out. Vecs have to have
            // all the same type so using Default
    ...
    pub fn pop(&mut self) -> Option<T> {
        ...
        // Removes an item at an index and
        // fills in with the last item of the Vec
...
mod tests {
    ...
    #[test]
    fn test_key_heap() {
        let mut heap: Heap<Point> =
            Heap::new(|a, b| a.0 < b.0);
        heap.add(Point(1, 5));
        heap.add(Point(3, 10));
        heap.add(Point(-2, 4));
    ...
    fn test_iter_heap() {
        ...
        // test iterator, which is not in order except the
        // first one.
