    ...
    pub fn solve_n_queens(n: i32) -> Vec<Vec<String>> {
        ...
        fn toggle_queen(
            ...
            for dir in directions {
                ...
                while i >= 0
                    && i < chessboard.len() as isize
                    && j >= 0
                    && j < chessboard.len() as isize
                {
                    chessboard[i as usize][j as usize] +=
                        toggle;
                    i += dir.0;
                    j += dir.1;
                }
        ...
        fn copy_board(
            chessboard: &Vec<Vec<i8>>,
        ) -> Vec<String> {
            let mut copy: Vec<char> =
                vec!['.'; chessboard.len()];
            let mut board: Vec<String> =
                Vec::with_capacity(chessboard.len());
        ...
        fn backtrack(
            ...
            for col in 0..chessboard.len() {
                if chessboard[row][col] == 0 {
                    toggle_queen(
                        row, col, chessboard, directions, 1,
                    );
                    backtrack(
                        chessboard,
                        result,
...
mod tests {
    ...
    #[test]
    fn test_solve_n_queens_n_4() {
        let n = 4;
        let mut results = Solution::solve_n_queens(n);
        let expected = vec![
            vec![
                ".Q..".to_string(),
                "...Q".to_string(),
        ...
        // Since the order of solutions is not important,
        // we sort the results for comparison
        results.sort();
        let mut sorted_expected = expected;
        sorted_expected.sort();

        assert_eq!(results, sorted_expected);
    }

    #[test]
    fn test_solve_n_queens_n_1() {
        let n = 1;
        let results = Solution::solve_n_queens(n);
        let expected = vec![vec!["Q".to_string()]];

        // No need to sort for a single solution
